...
Ideas captured during implementation of `repo-tools worktree merge` that are out of scope for the initial release.

## Rebase support

`repo-tools worktree rebase <branch>` — same workflow as merge but using rebase instead. Would share most of the state management, journal, and topology infrastructure.
