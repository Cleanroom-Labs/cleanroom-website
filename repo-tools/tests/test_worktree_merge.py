"""Tests for repo_tools.worktree_merge."""
...
from repo_tools.repo_utils import RepoInfo, discover_repos, run_git
from repo_tools.worktree_merge import (
    MergeState,
    RepoMergeEntry,
...
from repo_tools.config import MergeConfig
...
class TestGetTestCommand:
    ...
    def test_local_config_fallback(self, tmp_submodule_tree: Path):
        """Repo's own .repo-tools.toml should be used if no override."""
...
class TestStartMerge:
    def test_full_merge(self, tmp_submodule_tree_with_branches: Path):
        ...
        with patch("repo_tools.worktree_merge.find_repo_root", return_value=root):
...
class TestMergeJournalIntegration:
    def test_journal_entries_logged(self, tmp_submodule_tree_with_branches: Path):
        ...
        journal = git_dir / "repo-tools" / f"merge-journal-{now.strftime('%Y-%m')}.log"
