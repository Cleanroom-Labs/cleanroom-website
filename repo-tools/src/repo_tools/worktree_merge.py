...
from repo_tools.config import MergeConfig, load_config
...
from repo_tools.topology import TopologyCache, diff_snapshots
...
def _get_state_path(repo_root: Path) -> Path:
    ...
    return _get_git_worktree_dir(repo_root) / "repo-tools" / "merge-state.json"
...
def _get_journal_path(repo_root: Path) -> Path:
    ...
    return _get_git_common_dir(repo_root) / "repo-tools" / filename
...
def _execute_merge_for_repo(
    ...
    if result.returncode != 0:
        ...
        if _auto_resolve_submodule_conflicts(repo, merged_child_rel_paths):
            ...
            print(f"  Then run: repo-tools worktree merge --continue")
            return 1
    else:
    ...
    if not state.no_test:
        ...
        if test_cmd:
            ...
            if passed:
                ...
                print(f"    Fix the issue, then run: repo-tools worktree merge --continue")
                return 1
    ...
    if paused_entry.reason == "conflict":
        ...
        if unmerged:
            ...
            for f in unmerged:
                print(f"  - {f}")
            print()
            print("Resolve conflicts, then run: repo-tools worktree merge --continue")
            return 1
...
def run(args) -> int:
    ...
    if not branch:
        print(Colors.red("Usage: repo-tools worktree merge <branch>"))
