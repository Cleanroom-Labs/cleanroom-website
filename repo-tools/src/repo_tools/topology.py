...
The cache is stored at .git/repo-tools-topology.json and incrementally
...
def build_entries(repos, repo_root: Path) -> list[SubmoduleEntry]:
    ...
    """
    from repo_tools.repo_utils import parse_gitmodules, run_git
...
class TopologyCache:
    """Manages the .git/repo-tools-topology.json cache."""
    ...
    def for_repo(cls, repo_root: Path) -> TopologyCache:
        ...
        """
        from repo_tools.repo_utils import run_git
        ...
        return cls(git_dir / "repo-tools" / "topology.json")
